//! `neb migrate`: bring a v1 corpus forward to v2.
//!
...
use crate::corpus::config::{self, Config};
use crate::corpus::{Closed, Doc, Edge, EdgeType, Node, Origin, Reference, Status, Store};
use crate::corpus::{model, store};
...
/// Convert every node and the config in place, printing what changed.
pub fn migrate(root: Option<PathBuf>) -> Result<()> {
    let root = Store::resolve_root(root)?;
    ...
    for path in &paths {
        ...
        let (front, body) =
            model::split_frontmatter(&raw).with_context(|| format!("in {}", path.display()))?;
        let v1: V1Node = serde_yaml_ng::from_str(front)
            .with_context(|| format!("parsing {} with the v1 model", path.display()))?;
        let (node, notes) = convert(v1).with_context(|| format!("in {}", path.display()))?;
        let doc = Doc {
            node,
            body: body.to_string(),
        };
        ...
        changed += 1;
        println!("{}", bold(&doc.node.id));
        for n in &notes {
            println!("  {n}");
        }
    }

    let config_changed = migrate_config(&root)?;
    if config_changed {
        println!(
            "{} schema_version -> {}",
            bold(config::FILE),
    ...
    if changed == 0 && !config_changed {
        println!("{}", dim("already at schema 2; nothing changed"));
    } else {
        println!(
            "\n{}",
            dim(&format!(
                "{changed} of {} nodes rewritten; run `neb check` to confirm",
                paths.len()
            ))
        );
    }
    ...
    fn edges(&mut self, edges: Vec<V1Edge>) -> Result<Vec<Edge>> {
        ...
        for e in edges {
            let kind = match e.kind.as_str() {
                ...
                other => bail!("edge type `{other}` is not a v1 edge type"),
            };
...
fn convert_status(
    ...
    Ok(match status {
        ...
        other => bail!("status `{other}` is not a v1 status"),
    })
}
