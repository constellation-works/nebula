...
When touching `crates/nebula-core/src/{model,ops,check}.rs`, note that
invariants live in three places by design: deserialization (`model`), the
point of action (`ops`), and the checker (`check`). Moving a rule between them
changes its strength. The reasoning per rule is in
