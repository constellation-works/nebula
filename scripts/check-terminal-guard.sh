...
PATTERN='io::stdout|io::stderr|println!|eprintln!|print!|eprint!|dbg!'
...
if [[ -n "$hits" ]]; then
  echo "$hits" >&2
  echo "terminal-guard: std streams belong to $OUTPUT.rs only; write through its out!/outln!/errln!" >&2
  exit 1
fi
